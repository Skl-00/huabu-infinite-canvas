...
export async function hydrateCanvasImages(nodes: CanvasNodeData[]) {
    return Promise.all(
        nodes.map(async (node) => {
            ...
            const images = await Promise.all((metadata.images || []).map(async (image) => (image.storageKey ? { ...image, content: await resolveImageUrl(image.storageKey, image.content) } : image)));
