...
function siteToolSummary(name: string, result: unknown, input: unknown) {
    ...
    if (name === "generation_get_status") {
        ...
        return [
            tr("generationStatus", { total: numberField(data, "total"), queued: numberField(summary, "queued"), running: numberField(summary, "running"), succeeded: numberField(summary, "succeeded"), failed: numberField(summary, "failed") }),
            i18n.t("taskStatus.partialCount", { count: numberField(summary, "partial") }),
            i18n.t("taskStatus.interruptedCount", { count: numberField(summary, "interrupted") }),
        ].join(" · ");
    }
