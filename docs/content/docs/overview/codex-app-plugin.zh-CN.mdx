---
title: Codex App 插件
description: 在 Codex App 中安装并使用 HuaBu 画布插件
---

# Codex App 插件

HuaBu 画布提供了一个 Codex App 插件，用来把 Codex 和本地 Canvas Agent 连接起来。安装后，你可以直接在 Codex 中说“打开 HuaBu 画布”，让 Codex 打开画布、读取节点、创建节点并触发生成流程。
...
```text
请从 https://github.com/Skl-00/huabu-infinite-canvas.git 安装 HuaBu 画布 Codex 插件。
...
确保 `~/.agents/plugins/marketplace.json` 中有 HuaBu 画布条目。这里的 `path` 要指向仓库里的插件子目录：
...
如果你正在 HuaBu 画布仓库中开发插件，可以直接使用仓库自带的 marketplace：
...
```text
打开 HuaBu 画布新建画布并连接好 Codex
```

插件会启动本地 Canvas Agent，读取连接地址和 token，然后打开已配置的 HuaBu 画布前端并自动新建、连接画布。只有明确要求使用本地项目时，插件才会启动本地前端。
