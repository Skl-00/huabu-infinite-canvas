...
description: Install and use the HuaBu Canvas plugin in Codex App
---

# Codex App Plugin

The plugin connects Codex to a local Canvas Agent, allowing Codex to open HuaBu Canvas, read and create nodes, connect nodes, and start generation workflows. Its source is in `plugins/infinite-canvas/`.
...
```text
Install the HuaBu Canvas Codex plugin from https://github.com/Skl-00/huabu-infinite-canvas.git.
...
```bash
mkdir -p ~/plugins
git clone https://github.com/Skl-00/huabu-infinite-canvas.git ~/plugins/infinite-canvas
...
In a new Codex conversation, ask it to open HuaBu Canvas, create a canvas, and connect Codex. The plugin starts Canvas Agent, reads its connection URL and token, and opens the configured HuaBu Canvas frontend. It starts a local frontend only when explicitly requested.
