...
+ [新增] 设置页新增本地数据备份与恢复，可迁移画布、资产、四类持久任务、旧版历史和本地媒体；导入会重建媒体引用并保护 API 配置与凭据。
+ [修复] 本地备份导入增加清单/文件校验、失败回滚和媒体清理，避免无效或中断恢复留下半套数据。
+ [新增] 视频工作台接入独立持久任务仓储、跨标签执行锁、冻结参数重试与图片/视频任务中心，并联动首页、Agent 状态和媒体引用保留。
+ [修复] 视频任务在存储故障时保留远端身份和已取得结果，可重新保存而不再次生成；含凭据的 Gemini 下载地址不再作为远程结果保存。
+ [调整] 画布图片与视频生成复用持久任务执行器，保存项目/场景/节点上下文并在刷新后回填结果，复制场景不继承执行身份。

+ [新增] 增加 Agnes Flash 三模型预设及专用图片/视频协议，支持真实生成、原渠道视频恢复、公网参考图链接和远程视频本地保存。
+ [调整] Agnes Video Flash 的分辨率、时长和参考素材限制按官方协议在界面与请求适配器中统一校验。
+ [修复] 视频提交先保存记录，查询中断不重复生成，素材保存等待完成，删除历史不删除共享媒体，并修复手机视频页横向溢出。

+ [新增] 图片工作台接入持久化任务记录，支持逐张结果、部分成功、刷新中断恢复、原参数重试和跨页面结果复用。
+ [修复] 图片任务与素材操作确认保存结果，保留接口错误和任务引用图片，并修复手机顶栏重叠及生成记录入口不一致。
+ [调整] 画布插件的图片、视频、音频和文本生成统一复用持久任务仓储，保存来源节点上下文并进入任务中心。
+ [修复] 修复插件节点在 React 开发模式重挂载和刷新恢复时重复提交生成的问题；失败/中断任务不自动重发，主动新尝试需显式允许。
+ [调整] 图片多槽位任务改为逐槽位串行请求，兼容 Agnes 免费模型的单请求边界并继续保留部分成功结果。
+ [优化] 四类任务中心显示独立任务、画布节点和插件节点来源，插件任务可看到插件 ID、节点类型并返回来源画布。
+ [修复] 修复画布正反向拉线、右键创建、指针取消和重复连线行为，点击左侧创建工具后立即生成节点。
+ [新增] 增加卡藏式工作区层级，支持多工作画布创建、切换、复制、重命名、删除和整组导入导出，并在生成进行时保护导航。
+ [优化] 重做画布顶部、左侧创建栏、底部工具、资源面板、节点编辑器和移动端布局，使其基于 Infinite Canvas 原生底座对齐卡藏工作流。
+ [修复] 清理主页快速开始说明中的旧伪 HTML 标记，避免标签名称直接显示给用户。
+ [调整] 将站内 GitHub、版本检查、插件注册表、部署工作流和项目文档统一切换到 HuaBu 自有仓库。
+ [修复] 画布参考资源执行链改为识别持久化 `storageKey`，并在真实 Provider 请求前恢复为可传输的 Data URL，修复参考栏有内容但请求未携带图片的问题。
+ [修复] 画布 Config/Group/`@` 参考输入统一进入真实执行图，按资源节点去重，并补齐 OpenAI/Gemini 视频图片、视频、音频请求体。
+ [修复] 插件文本生成现在传递参考图片并使用可跨 Provider 传输的 Data URL，避免任务记录成功但多模态请求实际只携带 blob 地址。
+ [修复] 文本任务收到取消信号后立即持久化中断状态，保留已接收内容并阻止迟到的 Provider 结果覆盖任务状态。
